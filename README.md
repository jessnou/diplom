...
### Проблема
Несмотря на наличие правил дорожного движения и опыта водителей, автобусы остаются подвержены риску выезда за пределы полосы движения, особенно в сложных условиях: при плохой видимости (ночь, дождь, снег), изношенной или частично отсутствующей разметке, а также в условиях высокой плотности движения. Традиционные методы контроля (ручное наблюдение, GPS-трекинг) не позволяют своевременно обнаруживать отклонение от полосы и предупреждать водителя. Это ведёт к:
- повышенному риску ДТП и травматизма пассажиров;
- штрафам и репутационным потерям для автопарка;
- снижению эффективности мониторинга работы водителей.

Система LDW с использованием компьютерного зрения решает эту проблему, обеспечивая автоматическое, точное и оперативное обнаружение отклонения автобуса от полосы движения в реальном времени.
...
- Точность детекции разметки: Precision, Recall ≥ 0.95  (нужно или нет?)
- Время реакции системы: Latency ≤ 200 мс на кадр  (нужно или нет?)
...
- Обработка видеопотока в реальном времени (в качестве mvp обычное видео)
- Запись видео на сервер
- замывание номеров и лиц (в случиа хранения видео)

**Что не входит:**  
- Распознавание лиц или номеров  

## 7. UML-диаграммы (текстовое описание)
...
В качестве датасета дл обучения использовать https://bair.berkeley.edu/blog/2018/05/30/bdd/ , тип Lane Markings
