...
def convert_model(model_path, model_type=LaneModelType.UFLDV2_CULANE):
	...
	if ( "UFLDV2" in model_type.name) :
		...
		config_path = script_dir / "ufldDetector" / "exportLib" / "ultrafastLaneV2" / "configs" / f"{file.stem}.py"
		if not config_path.exists():
			raise FileNotFoundError(
				f"Config file not found: {config_path}\n"
				f"Hint: this path is derived from model filename '{file.name}'. "
				f"Either rename the model to match a config (e.g. tusimple_res18.pth), "
				f"or update convertPytorchToONNX.py to point at the desired config."
			)
		cfg = LaneV2Config(str(config_path))
...
if __name__ == '__main__':
	model_path = r"/home/jessnou/TSU/diplom/Vision-Guard-ADAS/TrafficLaneDetector/ufldDetector/exportLib/ultrafastLaneV2/tusimple_res18.pth" # Path to your PyTorch model file
